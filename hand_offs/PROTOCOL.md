# FIXED RELAY MESSAGE — CANONICAL COPY

**This block is fixed. Every hand-off must contain it verbatim.**
Canonical copy lives here: `hand_offs/PROTOCOL.md` — committed to the repo.

**v2 correction (2026-08-18, per Director):** handoffs are shared in the
repository. All handoffs go to the `hand_offs/` folder — every role,
**including the director's folder** (`hand_offs/director/`). They are not
kept local-only.
